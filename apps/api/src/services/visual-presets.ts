...
export const GRAPHIC_STYLE_PRESETS: PresetOption[] = [
  {
    ...
    promptHint: [
      "Create a stunning, cohesive vector icon system that feels like it was designed by the Stripe or Linear design team — pixel-perfect, optically harmonious, and gorgeous at every scale.",
      "",
      "ATMOSPHERE & MOOD:",
      "- Evoke the craftsmanship of a world-class product design system: every curve considered, every stroke deliberate.",
      "- The icons should feel like they belong together — a unified visual language that communicates 'premium software.'",
      "- Think: Phosphor Icons meets Apple SF Symbols — refined, balanced, and effortlessly readable.",
      "",
      "STYLE:",
      "- Stroke-first vector style with precisely consistent line weight (1.75-2px) across every single icon — no exceptions.",
      "- {{primary_dark}} for all primary icon outlines and structural strokes.",
      "- {{primary_accent}} as a single accent fill or highlight, used exclusively for active/selected/emphasized states.",
      "- Unified corner radius language: 2px for small elements (<8px), 4px for medium (8-16px), 6px for large elements (>16px).",
      "- Flat fills only — absolutely no gradients, shadows, glows, or 3D effects within icons.",
      "- Cap style: round caps and round joins throughout for warmth and approachability.",
      "- Optical correction: circles should be slightly larger than squares, pointed shapes extend 0.5px beyond grid edges for visual alignment.",
      "",
      "GRID & SIZING SYSTEM:",
      "- Master grid: 24x24px base with 2px safe padding (20x20px live area).",
      "- Inner guides: 14x14px key shape area for primary forms, 18x18px for full-bleed fills.",
      "- Pixel alignment: all strokes and anchor points snap to whole-pixel grid — no sub-pixel rendering.",
      "- Weight variants: each icon should work at 1.5px (thin), 2px (regular), and 2.5px (bold) stroke weights.",
      "",
      "STRUCTURE REQUIREMENTS:",
      "- Icon categories with clear visual family resemblance:",
      "  • Navigation (4-6 icons): arrows, home, menu, search — foundational wayfinding.",
      "  • Actions (4-6 icons): edit, delete, share, download, copy — direct manipulation.",
      "  • Status (4-6 icons): check, warning, info, error, loading — system feedback.",
      "  • Data/Chart (4-6 icons): bar chart, pie, trending, table, filter — analytics vocabulary.",
      "- Each category should be visually distinguishable but share the same DNA.",
      "- Variant states: outlined (default), filled (active/selected), and duo-tone (outlined with {{primary_accent}} fill accent) for core navigation icons.",
      "",
      "TYPOGRAPHY:",
      "- Labels: {{font_label}}, 8-9pt, in {{primary_dark}} tone, centered 4px below each icon.",
      "- Category headers: {{font_subhead}}, 11pt, {{primary_dark}}, left-aligned above each row.",
      "",
      "VISUAL RULES:",
      "- No open endpoints — all strokes terminate at intersections, corners, or rounded caps.",
      "- Maximum 2 stroke weights per individual icon (primary structural + fine detail).",
      "- Negative space must be clean and generous — avoid overly complex or busy internal shapes.",
      "- Mental test: render each icon at 16px — if ANY detail becomes muddy, simplify immediately.",
      "- Consistent visual weight: all icons should feel the same 'heaviness' when viewed as a set.",
      "- No decorative flourishes — each stroke must serve a functional purpose.",
      "",
      "PRESENTATION LAYOUT:",
      "- Display icons in an organized grid with 40px spacing between icons.",
      "- Group by category with subtle {{primary_light}} background strips (8px vertical padding) behind each row.",
      "- Include a variant showcase: one icon shown in outlined, filled, and duo-tone variants side by side.",
      "",
      "MUST INCLUDE:",
      "- Obsessively consistent stroke thickness, cap style, and corner radii across ALL icons.",
      "- Clear positive/negative space contrast even at smallest sizes.",
      "- Verified readability at 16px, 24px, and 32px render sizes.",
      "- At least 16 distinct, purposeful icons across 4 categories.",
      "- Outlined + filled variant states for at least 4 core icons.",
      "- The quality feel of a professional icon library you'd pay $200+ for."
    ].join("\n")
  },
  {
    id: "flat_infographic",
  ...
  },
  {
    ...
    promptHint: [
      ...
      "- Dramatic gradient background with noise texture and optional light leak.",
      "- Safe margins for social crop tolerance on all platforms.",
      "- Feed-stopping visual impact that works at BOTH thumbnail and full-screen scale.",
  ...
  },
  {
    ...
    promptHint: [
      ...
      "",
      "VISUAL RULES:",
      "- Realistic UI proportions — elements must look exactly like a production application screenshot.",
      "- Consistent 8px spacing grid for ALL padding, margins, and gaps — pixel-perfect alignment.",
      "- No decorative elements that wouldn't exist in a real SaaS application.",
      "- Layer hierarchy: nav bar (z-index high) > sidebar > content cards > background — depth cues must be clear.",
      "- Interaction states visible: at least one hovered button, one active nav item, one focused input field.",
      "- The interface should feel alive — not a static wireframe but a snapshot of a working product.",
      "",
      "MUST INCLUDE:",
      "- Distinct navigation/header strip with realistic controls (search, user avatar, notifications).",
      "- At least one chart or data-visualization block with realistic data.",
      "- At least one data table with realistic entries and interaction states.",
      "- KPI cards with metrics, sparklines, and delta indicators.",
      "- Clear panel hierarchy with multi-layer depth cues (shadows, borders, background tints).",
      "- One interactive overlay element (modal, dropdown, or tooltip) showing UI depth.",
      "- Production-quality fidelity — this should be indistinguishable from a real application screenshot."
    ].join("\n")
  },
  {
    id: "sticker_badge_3d",
