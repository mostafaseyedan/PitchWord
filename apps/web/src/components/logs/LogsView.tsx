...
export const LogsView = ({ analytics, logs }: LogsViewProps) => {
  return (
    <div className="space-y-6">
      ...
      <div className="mt-4 pt-10 border-t border-border-warm/30">
        ...
        <div className="max-h-[500px] overflow-y-auto pr-1">
          {logs.slice(0, 100).map((log, index) => (
            <div
              key={log.id}
              className={`px-5 py-4 transition-all border-b border-border-warm/10 last:border-0 hover:bg-page/40`}
            >
              <div className="flex items-center justify-between gap-4 mb-1">
                <span className="text-[15px] font-medium text-primary">{log.stepName}</span>
